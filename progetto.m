...
x2_eq = Tamb + (Tdiff/c)*(log(K_star/(K_max - K_star))); % temperatura per tenere il peso a 4cm
dK_dx2 = ((c*K_max)/Tdiff)*(exp(c*(x2_eq-Tamb)/Tdiff)/(1+(exp(c*(x2_eq-Tamb)/Tdiff)^2))); % altro parametro per la matrice
x1_eq = sqrt((1/Re)*(h*S*(x2_eq - Tamb)+(x2_eq-Tp)/Rth)); % Corrente equivalente 
K = K_max*(1-(1/(1+exp(c*((x2_eq-Tavg)/Tdiff)))));

% Matrici
a11 = -((h*S+(1/Rth))/Cth)
a31 = (r1/r2)*((dlmax-((r1/r2)*z_star))/(m+(J/((r2)^2))))
